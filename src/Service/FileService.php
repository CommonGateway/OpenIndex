...
class FileService
{
    ...
    public function getInhoudDocument(string $caseId, string $documentId, string $mimeType, Source $zaaksysteem): ?string
    {
        try {
            ...
            $this->logger->info("Fetching inhoud document: $documentId for case $caseId", ['plugin' => 'common-gateway/open-index']);
            ...
            $this->logger->error("Failed to fetch inhoud of document: $documentId, message:  {$e->getMessage()}", ['plugin' => 'common-gateway/open-index']);
            return null;
        }
