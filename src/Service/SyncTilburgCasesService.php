...
use CommonGateway\OpenIndex\Service\FileService;
 ...
 *
 * @author   Acato BV <yoeri@acato.nl>, Wilco Louwerse <wilco@conduction.nl>
 * @category Service
 */
class SyncTilburgCasesService
{
    ...
    public function SyncTilburgCasesHandler(array $data, array $configuration): array
    {
        ...
        $source  = $this->resourceService->getSource($this->configuration['source'], 'common-gateway/open-index');
        $schema  = $this->resourceService->getSchema($this->configuration['schema'], 'common-gateway/open-index');
    ...
    private function processAttachment(array $attachment, Source $source, Mapping $mapping, Endpoint $endpoint): array|bool
    {
        ...
        try {
            ...
            $this->logger->error(var_export([ 'ERROR fileService' => $e->getMessage() ], true), [ 'plugin' => 'common-gateway/open-index' ]);
        ...
        if ($extension === 'pdf') {
            try {
                ...
                $this->logger->error(var_export([ 'ERROR pdfParser' => $e->getMessage() ], true), [ 'plugin' => 'common-gateway/open-index' ]);
            }
        }
    ...
    private function savePublication(array $data, Source $source, Schema $schema, $flush=true, $unsafeHydrate=false): ObjectEntity|bool
    {
        try {
            ...
            $this->logger->error(var_export([ 'ERROR' => $exception->getMessage() ], true), [ 'plugin' => 'common-gateway/open-index' ]);
        }
    ...
    private function map(Mapping $mapping, array $result): array
    {
        try {
            ...
            $this->logger->error(var_export([ 'ERROR mappingService' => $exception->getMessage() ], true), [ 'plugin' => 'common-gateway/open-index' ]);
            $mappedResult = [];
        }
    ...
    private function isValidData(array $result, Schema $schema, string $method='POST'): bool
    {
        try {
            ...
            if (null !== $validationErrors) {
                $this->logger->error(var_export([ 'validationErrors' => $validationErrors ], true), [ 'plugin' => 'common-gateway/open-index' ]);

                return false;
            }
        } catch (Exception $exception) {
            $this->logger->error(var_export([ 'ERROR validationService' => $exception->getMessage() ], true), [ 'plugin' => 'common-gateway/open-index' ]);
    ...
    private function fetchObjects(Source $source, string $start, string $end): array
    {
        try {
            ...
            $this->logger->error('Something went wrong fetching '.$source->getLocation().$this->configuration['caseIndex'].': '.$e->getMessage(), [ 'plugin' => 'common-gateway/open-index' ]);
    ...
    private function fetchDetails(Source $source, string $identification): array
    {
        try {
            ...
            $this->logger->error('Something went wrong fetching '.$source->getLocation().$endpoint.': '.$e->getMessage(), [ 'plugin' => 'common-gateway/open-index' ]);
